...
public:
    ...
    void handleInputEvent(EvDev const* src, struct input_event const ev[], size_t const count);
private:
    void processKbdInput(struct input_event const ev[], size_t const count);
    void processMouseInput(struct input_event const ev[], size_t const count);
private:
